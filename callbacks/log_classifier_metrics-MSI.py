    ...
    def _shared_eval(self, trainer, prefix):
        ...
        for metric_name in self.metric_names:
            if metric_name in self.metric_dict:
                ...
                if metric_name == 'accuracy' and metric_val > self.highest_acc:
                    self.highest_acc = metric_val
                    self.log(f"{prefix}_best_acc", metric_val)
                elif metric_name == 'f1-score' and metric_val > self.highest_f1:
                    self.highest_f1 = metric_val
                    self.log(f"{prefix}_best_f1", metric_val)
