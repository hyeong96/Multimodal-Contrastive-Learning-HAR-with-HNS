...
class MMHarDataModule(LightningDataModule, metaclass=ABCMeta):
    ...
    def _init_dataloaders(self):
        ...
        subset = list(range(0, len(train_dataset), int(100/self.percentage)))
        train_subset = torch.utils.data.Subset(train_dataset, subset)
        print("train", len(train_dataset), len(train_subset))
        test_dataset = self._create_test_dataset()
        drop_last_ssl = bool(self.ssl)
        self.train = DataLoader(train_subset, batch_size=self.batch_size, shuffle=True, drop_last=drop_last_ssl, num_workers=self.num_workers, pin_memory=True)
        ...
        if "val" in self.split:
            ...
            subset = list(range(0, len(val_dataset), int(100/self.percentage)))
            val_subset = torch.utils.data.Subset(val_dataset, subset)
            print("train", len(val_dataset), len(val_subset))
            self.val = DataLoader(val_subset, batch_size=self.batch_size, shuffle=False, drop_last=drop_last_ssl, num_workers=self.num_workers, pin_memory=True)
        else:
            self.val = None
