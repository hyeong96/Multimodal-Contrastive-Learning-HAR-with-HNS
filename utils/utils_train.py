...
from models.simclr_hn import SimCLRUnimodal
...
from callbacks.log_confusion_matrics import LogConfusionMatrix
...
from utils.utils_exp import load_yaml_to_dict
...
def init_datamodule(data_path, dataset_name, modalities, batch_size,
                    ...
                    ssl = False, n_views = 2, num_workers = 1, percentage = 100, limited_k=None):
    ...
    data_module = dataset_properties.datamodule_class(path = data_path, modalities = modalities, batch_size = batch_size, 
        ...
        ssl = ssl, n_views = n_views, num_workers = num_workers, percentage = percentage, limited_k = limited_k)
    return data_module
...
def setup_wandb_logger(experiment_info, modality, dataset, experiment_id, entity='transformationnetwork', approach='supervised'):
    return loggers.WandbLogger(config=experiment_info, entity=entity, project="Contrastive Learning with Hard Negatives for Multimodal HAR", name=experiment_id, id=experiment_id)


def setup_loggers(logger_names=['tensorboard', 'wandb'], tb_dir=None, experiment_info=None, modality=None, dataset=None, 
        experiment_id=None, entity='transformationnetwork', approach='supervised', experiment_config_path=None):
    loggers = []
    loggers_dict = {}
